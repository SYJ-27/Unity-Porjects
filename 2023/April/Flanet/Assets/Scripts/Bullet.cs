...
public class Bullet : MonoBehaviour
{
    ...
    void Update()
    {
        if(transform.position.x != 0 || transform.position.y != 0){
            var angleTemp = Mathf.Atan2(earthTransform.position.y - transform.position.y, earthTransform.position.x - transform.position.x) * Mathf.Rad2Deg;
    ...
    {
        if(other.tag == "Player"){
            GameManager.gameManager.SetScoreEnemy(0);
            Destroy(gameObject);
        }
    }
